...
def main():
 ...
 pop = simple_GA.population(n=NPOP, k= CHROM_SIZE)
 pop.random_populate()
 pop.evaluate()
 pop.view(sort=True)
 ...
 for gen in range(1,NGEN):
  ...
  plt.xlim(1,NGEN)
  plt.pause(0.05)
  new_pop = simple_GA.population(n=NPOP, k= CHROM_SIZE)
  for _ in range(NPOP):
   c = pop.select(tournsize=4,k=2)
   offspring = simple_GA.cross_over(c[0],c[1],CROSS_PROB)
   c = simple_GA.mutate_chromosome(offspring,MUT_PROB)
